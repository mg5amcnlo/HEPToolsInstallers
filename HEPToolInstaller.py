...
_HepTools = {'hepmc':
             ...
             'hepmc3':
               {'install_mode':'Default',
                ...
                'allow_system_wide': False,
                'version':       '3.3.1',
                'www': 'https://gitlab.cern.ch/hepmc/HepMC3/-/archive',
                'tarball':      ['online','%(www)s/%(version)s/HepMC3-%(version)s.tar.gz'],
                'mandatory_dependencies': [],
                'optional_dependencies' : [],
...
def finalize_installation(tool):
    ...
    if tool in ['pythia8', 'ninja']:
        ...
        #avoid relinking of libavh_olo.a
        all_lib = [lib for lib in all_lib if not lib.endswith('libavh_olo.a')]
